(function (context) {
    ...
    var initPage = function (config) {
        ...
        // compute logo's location:logoCountͼƬ�ĸ���logoͼƬ��ID
        var logoCount = self.config[CONFIG_LOCATION].length / 4, i, logo;
        var rawRatio, wRatio, topRatio, leftRatio;
        for (i = 0; i < logoCount; i++) {
            logo = $(pageId + '-logo' + i);

            rawRatio = self.config[CONFIG_LOCATION][4 * i]; // ͼƬ w/h
            wRatio = self.config[CONFIG_LOCATION][4 * i + 1]; // ͼƬ w/W
            topRatio = self.config[CONFIG_LOCATION][4 * i + 2]; // ͼƬ top/H
        ...
        var configs = [
            {
                index: 0,
                location: [
                    //page-0-logo0-old,page-0-logo-1-old
                    //1.0455, 0.2875, 0.220951, -999,
                    //3.14474, 0.3734375, 0.42077, -999,
                    1,0.2125,0.2200704,-999,
                    3.0860215,0.4484375,0.39172535,-999,
            ...
            },
            {
                ...
                location: [
                    ...
                    0.56962, 0.2109375, 0.471831, -0.075,//leftΪ��ֵ
                    0.625616, 0.1984375, 0.5017606, 0.3578125,
                    0.8448276, 0.30625, 0.46831, 0.09375,
                    0.549107, 0.1921875, 0.55986, -0.059375,//leftΪ��ֵ
